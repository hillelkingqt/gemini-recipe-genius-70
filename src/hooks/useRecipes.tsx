...
export const useRecipes = () => {
        ...
        try {
            ...
            const loadedRecipes: Recipe[] = data.map(item => ({
                ...
                status: ['draft', 'accepted', 'rejected', 'published'].includes(item.status)
                    ...
                    : 'draft',
                timeMarkers: Array.isArray(item.time_markers)
                    ? item.time_markers.map((tm: { step: number; duration: number; description: string }) => ({
                        step: Number(tm.step),
                        duration: Number(tm.duration),
                ...
                nutritionInfo: item.nutrition_info as {
                    calories?: string;
                    protein?: string;
                    carbs?: string;
                    fat?: string;
                } || {},
                seasonality: Array.isArray(item.seasonality)
                    ? item.seasonality.map(s => String(s))
                    : [],
                ...
                user_id: item.user_id, // אל תשכח אותו
            }));


            setRecipes(loadedRecipes);
            return loadedRecipes; // ← החזר אותם
        } catch (error) {
    ...
    const addRecipe = async (recipe: Omit<Recipe, 'id' | 'notes' | 'rating' | 'status' | 'createdAt' | 'isFavorite'> & { status?: 'draft' | 'accepted' | 'rejected' | 'published' }, status = 'accepted') => {
        ...
        try {
            ...
            const newRecipe: Recipe = {
                ...
                user_id: data.user_id, // ✅ הוספה חשובה
            };
    ...
    const fetchCommunityRecipes = useCallback(async () => {
        console.log("fetchCommunityRecipes called");
        try {

            const published = await fetchPublishedRecipes();
            setCommunityRecipes(published);
        } catch (error) {
    ...
    const fetchPublishedRecipes = async (): Promise<Recipe[]> => {
        try {
            ...
            const publicRecipes: Recipe[] = data.map(item => ({
                ...
                author: item.author || '',
                imageBase64: item.image_base64 || '',
                user_id: item.user_id, // ✅ הוספה חשובה!
            }));
    ...
    return {
        ...
        fetchUserRecipes,       // ✅ חדש
        fetchCommunityRecipes,  // ✅ חדש
        updateNotes,         
        toggleLike              // ✅ חדש
    };
